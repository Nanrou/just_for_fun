...
SA = namedtuple('SA', ['index', 'height', 'place', 'str'])
...
class AnalyseCommonPart:
    ...
    def analyze2(self):
        ...
        _all_text = '$'.join(self.texts)
        for index, sa in enumerate(self._sa):
            self.sa_list.append(SA(index, tmp_height[index], self.get_original_distribution(sa), _all_text[sa]))

        for item in self.sa_list:
            self.str_sa_mapper.setdefault(item.str, []).append(self._sa[item.index])

        res = dict()
        for item in self.sa_list:
            if item.height > 1:
                _strings = self.get_original_element(self._sa[item.index], item.height)
                index_list = {item.place}
                for other_sa in self.str_sa_mapper[item.str]:
                    if _strings == self.get_original_element(other_sa, item.height):
                        index_list.add(self.sa_list[self._rank[other_sa]].place)
                res[_strings] = sorted(list(index_list))
    ...
    # def lcp_substring_detail_hash(self):
    #     res = dict()
        # for strings, index_list in self.lcp_string:
...
if __name__ == '__main__':
    ...
    # time_clock(aa.analyze)()
    # time_clock(aa.lcp_substring_detail_rude)()
    ...
    #     time_clock(aa.lcp_substring_detail_for_test, 'test')()
    #
    # for _ in range(3):
    #     aa = AnalyseCommonPart(tt * 3)
    #     time_clock(aa.lcp_substring_detail_rude, 'rude')()
    ...
    #
    #     aa = AnalyseCommonPart(tt * 100, dc3=True)
