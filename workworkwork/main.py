...
def sheet_one(data):
    ...
    with xlsxwriter.Workbook('{}.xlsx'.format(data.get('title', 'test'))) as wb:
        ...
        worksheet.write_string('O9', DATA.get('直供:供水户数'), wb.add_format(tmp))
...
def sheet_two(data):
    ...
    with xlsxwriter.Workbook('{}.xlsx'.format(data.get('title', 'test'))) as wb:
        ...
        worksheet.merge_range('M6:O6', '物管单位', wb.add_format(tmp))
        ...
        for r, s in zip(range(3, 6), ['供水所', '维护单位', '物管单位']):
